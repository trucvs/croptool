...
class VideoCropApp:
    ...
    def _build_left_panel(self) -> None:
        with dpg.child_window(width=390, autosize_y=True, border=False):
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(label="Chon video", callback=lambda: dpg.show_item(self.tags["input_dialog"]), tag="browse_input_button")
            ...
            with dpg.group(horizontal=True):
                dpg.add_input_int(label="X", width=160, tag=self.tags["crop_x"], default_value=0, callback=self._on_crop_changed)
                dpg.add_input_int(label="Y", width=160, tag=self.tags["crop_y"], default_value=0, callback=self._on_crop_changed)
            with dpg.group(horizontal=True):
                dpg.add_input_int(
                    label="Width",
                    width=160,
    ...
    def _on_input_file_selected(self, _sender: str, app_data: dict, _user_data: object) -> None:
        ...
        dpg.set_value(self.tags["output_path"], str(self._suggest_output_path(input_path)))
    ...
    def _on_output_dir_selected(self, _sender: str, app_data: dict, _user_data: object) -> None:
        ...
        if input_text:
            suggested_name = self._suggest_output_path(Path(input_text)).name
        else:
    ...
    def _suggest_output_path(self, input_path: Path) -> Path:
    ...
    def _load_video(self, path: Path) -> None:
        ...
        try:
            self.metadata = self._probe_video(path)
        except Exception as exc:  # noqa: BLE001
            self.metadata = None
            self._delete_preview_assets()
            self._set_source_info("Khong the doc metadata tu video.")
        ...
        dpg.set_value(self.tags["crop_width"], self.metadata.width)
        dpg.set_value(self.tags["crop_height"], self.metadata.height)
    ...
    def _probe_video(self, path: Path) -> VideoMetadata:
        command = [self.ffmpeg_path, "-hide_banner", "-i", str(path)]
    ...
    def _refresh_source_info(self) -> None:
        ...
        duration = self._format_duration(self.metadata.duration_seconds)
        self._set_source_info(
            "\n".join(
                [
                    f"File: {self.metadata.path.name}",
                    f"Resolution: {self.metadata.width} x {self.metadata.height}",
                    f"Duration: {duration}",
                    f"Codec: {self.metadata.codec_name}",
                    f"FPS: {self.metadata.frame_rate}",
                ]
            )
        )
    ...
    def _refresh_preview_image(self) -> None:
        ...
        seek_seconds = min(self.metadata.duration_seconds or 0.0, 1.0)
        command = [
            self.ffmpeg_path,
            "-hide_banner",
            "-loglevel",
    ...
    def _render_preview(self) -> None:
        ...
        x1 = origin_x + crop_x * scale_x
        y1 = origin_y + crop_y * scale_y
        x2 = origin_x + (crop_x + crop_width) * scale_x
        y2 = origin_y + (crop_y + crop_height) * scale_y

        shade_fill = (2, 6, 23, 120)
        outline = (249, 115, 22, 255)
        dpg.draw_rectangle((origin_x, origin_y), (origin_x + image_width, y1), fill=shade_fill, color=shade_fill, parent=self.tags["preview_drawlist"])
        dpg.draw_rectangle((origin_x, y2), (origin_x + image_width, origin_y + image_height), fill=shade_fill, color=shade_fill, parent=self.tags["preview_drawlist"])
        dpg.draw_rectangle((origin_x, y1), (x1, y2), fill=shade_fill, color=shade_fill, parent=self.tags["preview_drawlist"])
        dpg.draw_rectangle((x2, y1), (origin_x + image_width, y2), fill=shade_fill, color=shade_fill, parent=self.tags["preview_drawlist"])
    ...
    def _update_crop_summary(self) -> None:
        crop = self._get_crop_values(strict=False)
        if not self.metadata:
            self._set_crop_summary("Tai video truoc de xem thong tin crop.")
            return

        if crop is None:
            self._set_crop_summary(
                f"Nhap x, y, width, height hop le nam trong frame {self.metadata.width}x{self.metadata.height}."
            )
            return

        x, y, width, height = crop
        self._set_crop_summary(f"Crop {width}x{height} tai ({x}, {y}) | Output frame: {width}x{height}")

    def _get_crop_values(self, strict: bool) -> Optional[tuple[int, int, int, int]]:
        if not self.metadata:
            return None
        ...
        if x + width > self.metadata.width or y + height > self.metadata.height:
            if strict:
    ...
    def _center_current_crop(self) -> None:
        ...
        centered_x = max((self.metadata.width - width) // 2, 0)
        centered_y = max((self.metadata.height - height) // 2, 0)
        dpg.set_value(self.tags["crop_x"], centered_x)
        dpg.set_value(self.tags["crop_y"], centered_y)
        self._update_crop_summary()
        self._render_preview()
    ...
    def _apply_ratio_preset(self) -> None:
        ...
        source_ratio = self.metadata.width / self.metadata.height
        target_ratio = ratio_width / ratio_height

        if source_ratio > target_ratio:
            crop_height = self.metadata.height
            crop_width = int(math.floor(crop_height * target_ratio))
        else:
            crop_width = self.metadata.width
            crop_height = int(math.floor(crop_width / target_ratio))

        crop_width = min(crop_width, self.metadata.width)
        crop_height = min(crop_height, self.metadata.height)
        crop_x = max((self.metadata.width - crop_width) // 2, 0)
        crop_y = max((self.metadata.height - crop_height) // 2, 0)

        dpg.set_value(self.tags["crop_x"], crop_x)
        dpg.set_value(self.tags["crop_y"], crop_y)
        dpg.set_value(self.tags["crop_width"], crop_width)
        dpg.set_value(self.tags["crop_height"], crop_height)
        self._update_crop_summary()
        self._render_preview()
    ...
    def _start_crop(self) -> None:
        ...
        if not output_text:
            output_path = self._suggest_output_path(self.metadata.path)
        ...
        command = self._build_ffmpeg_command(self.metadata.path, output_path, crop)
    ...
    def _parse_ffmpeg_timestamp(self, value: str) -> float:
        try:
